...
public final class PulseVariable extends JavaPlugin {
    ...
    public void onEnable() {
        try { Register(this, "com.closeplanet2.pandavariablecore"); }
    ...
    private static void RegisterAllTests(List<Class<?>> classes, JavaPlugin plugin){
        ...
        for(var clazz : classes){
            ...
            for(var x : listener.CLASS_TYPE()) VAR_TESTS.put(x, listener);
            for(var y : listener.ARRAY_TYPE()) VAR_TESTS.put(y, listener);
            count += 1;
        }
        ConsoleAPI.SEND(ChatColor.GREEN + String.format("&aAll Tests Loaded! (%d)", count));
    }
}
