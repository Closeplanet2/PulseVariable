...
@VarTest
public class EffectAPI implements VariableLogic {
    @Override
    public boolean IS_TYPE(String variable) {
        try{
            var test = Effect.valueOf(variable);
            return true;
    ...
    @Override
    public Object DE_CONVERT_FOR_CONFIG(Object data) { return Effect.valueOf(data.toString()); }

    @Override
    public List<String> TAB_DATA(List<String> to_add, String currentArgument) {
        var data = new ArrayList<String>();
        for(var x : Effect.values()) data.add(x.toString());
        return data;
    }
