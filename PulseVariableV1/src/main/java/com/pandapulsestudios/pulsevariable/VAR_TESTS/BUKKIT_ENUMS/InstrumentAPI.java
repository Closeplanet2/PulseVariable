...
@VarTest
public class InstrumentAPI implements VariableLogic {
    @Override
    public boolean IS_TYPE(String variable) {
        try{
            var test = Instrument.valueOf(variable);
            return true;
    ...
    public Object DE_CONVERT_FOR_CONFIG(Object data) {
        return Instrument.valueOf(data.toString());
    }

    @Override
    public List<String> TAB_DATA(List<String> to_add, String currentArgument) {
        var data = new ArrayList<String>();
        for(var x : Instrument.values()) data.add(x.toString());
        return data;
    }
