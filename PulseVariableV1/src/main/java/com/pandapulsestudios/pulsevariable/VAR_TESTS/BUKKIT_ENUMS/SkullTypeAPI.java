...
@VarTest
@Deprecated
public class SkullTypeAPI implements VariableLogic {
    @Override
    @Deprecated
    public boolean IS_TYPE(String variable) {
        try{
            var test = SkullType.valueOf(variable);
            return true;
    ...
    public Object DE_CONVERT_FOR_CONFIG(Object data) {
        return SkullType.valueOf(data.toString());
    }
    ...
    public List<String> TAB_DATA(List<String> to_add, String currentArgument) {
        ...
        for(var x : SkullType.values()) data.add(x.toString());
        return data;
    }
    ...
    public void CUSTOM_CAST_AND_PLACE(List<Object> toAdd, int place, List<?> castedData, Class<?> arrayType) {
        toAdd.add(castedData.toArray(new SkullType[0]));
    }
}
